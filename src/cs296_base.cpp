...
void base_sim_t::step(settings_t* settings)
{
  ...
  m_point_count = 0;
  /*! Check whether there are some body which are to be destroyed and then destroy them in each step you do this*/
  while(!toBeDestroyed.empty()){
  	...
  	if (selfStart)
	{
		...
		for (int i=0; i< n; i++)
		{
		crank->ApplyTorque(-3000, true);} ///> Crank Rotate: Apply torque to crank 
  ...
  /*! If createfuel flag is true this will generate fuel particles with some initial velocity*/
  if(createfuel){
	  for (int i=0;i<2;i++){
		  b2Body *fuel;

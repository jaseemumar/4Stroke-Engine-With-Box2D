...
namespace cs296
{
  ...
  void callbacks_t::keyboard_cb(unsigned char key, int x, int y)
  {
    ...
    case 'p':
      ...
      break;
	/** A case that would allow you to apply breaks in to the system using 'b' key press*/
    case 'b':
		
		brakehandle->ApplyForce(b2Vec2(5000,0), brakehandle->GetWorldCenter(), true); /// > Apply force on brake handle
		braked=true; /// > Braked : Braked True means that 
		for(set<b2Body*>::iterator it=inCylFuelSet.begin();it!=inCylFuelSet.end();it++){
					(*it)->SetLinearVelocity(b2Vec2(0,0));
					
		}
		break;
	/*! KeyBoard event s starts the engine, when one long press s, in each step this case is matched and a torque is given to the crank
	 * which will start rotating it.*/
	case 's':
		crank->ApplyTorque(-1000, true); ///> Crank Rotate: Apply torque to crank 
		braked=false; /// > This will set the braked variable to false
    	break;

...
namespace cs296
{
	...
	/*! These variables are declared outside the constructor so that they are accesible in callbacks as well under the namespace*/
	b2Body* crank; /// > Crank : Rotaing body that connects to the blade-Gear
	b2Body *brakehandle; /// > Brake Handle : Handle used to pull brakes
	b2Body * square; /// > square sensor : When this square touch the sensor it creates more fuel
	b2Body * exhaust; /// > Exhuast : Smoke Pipe contains a filter that swallows all the particles
	b2Body * rightValve;
	b2Body * leftValve;
	b2Body * valveTrep;
	...
	b2Body * fueltank;
	b2Body* piston; /// > Piston : Central Trepazium that ocilates in y-direction
	b2Body*stroke2sensor; /// > Stroke Sensors : causes the density change of bodies
	...
	queue<b2Body*> toBeDestroyed; /// > toBeDestroyed : A list of bodies to be destroyed by the destructor
	float height_support = 15.0f;
	long long ntoBeDestroyed=0;
	extern set<b2Body*> fuelSet; /// > fuelSet : This is fuel that is there in the fueltank system
	extern set<b2Body*> inCylFuelSet; /// > inCylFuelSet : This is the fuel that is there in cylinder, only these will be resposible for pushing the piston down
	...
	/*! <b> Contact listener class </b> : It is instantiated to detect whether there 
	 ...
	 * along with piston becomes transparent and other such events that occurs in the system*/
	class MyContactListener : public b2ContactListener
	{	
		/*! Contact Listenser subclass*/
		...
		void BeginContact(b2Contact* contact) {
			...
			/// If one of the entity involved in collision is exhaust means that the other body is for sure the fuel particles in the exhaust pipe and shall be destroyed 
			if (entityA==exhaust){
				destroyfuel=true;
			...
			/// > when some body touches the square then fuel starts generating in the fuel chamber
			else if (entityA==square||entityB==square){
				createfuel=true;
			}
			/// > This will put fuel from the fuel set and transfer it to the incyl fuel set
			...
			/** Sensor B is true whenthe body B in collision is a sensor
			* Make the maskbit of the right valve to 0 and makes it transparent to the fuel*/
			...
			/*! This is the first sensor, which enables stroke two of engine, 
			 * Here piston is moving up and the particles density we have decreased*/
				...
				/*! Here we are using the braked variable to check whether the brakes are applied or not
				 * If brakes are not applied and when the ball in contact with the sensors we will increase the 
				 * velocity of the fuel in y-direction and also increase there density to push the piston downwards
				 * */
	...
	* </ul>*/
	...
	//chasis
		...
		b2Vec2 verticesChasis[8];
		verticesChasis[0].Set(chasiswidth/4 + 5, chasisheight+3);
		verticesChasis[1].Set(-3*chasiswidth/4, 3*chasisheight/4 + 3);
		verticesChasis[2].Set(-chasiswidth-5, chasisheight/4);
		verticesChasis[3].Set(-chasiswidth-5, -chasisheight/4);
		verticesChasis[4].Set(-3*chasiswidth/4, -3*chasisheight/4 - 3);
		verticesChasis[5].Set(chasiswidth/4 + 5, -chasisheight-3);
		verticesChasis[6].Set(chasiswidth, chasisheight);
		verticesChasis[7].Set(chasiswidth, -chasisheight);
		shape.Set(verticesChasis,8);
	...
	//Chasis
		...
		shape.SetAsBox(0.1f,chasisheight - 4,b2Vec2(50.0f,0.0f),0);
		bbox->CreateFixture(&fdbox);
		shape.SetAsBox(15.0f,0.1f,b2Vec2(30 + 5.0f,-chasisheight + 3.5 ),0);
		bbox->CreateFixture(&fdbox);
		shape.SetAsBox(15.0f,0.1f,b2Vec2(30.0f + 5,chasisheight - 3.5),0);
		bbox->CreateFixture(&fdbox);
		}
		...
		{
			b2PolygonShape shape;
			shape.SetAsBox(3.0f, 1.5);
			b2FixtureDef fdwheel;
			fdwheel.shape = &shape;
			fdwheel.filter.maskBits=8;
			b2BodyDef bdwheel;
			bdwheel.type=b2_dynamicBody;
			bdwheel.position.Set(boxcenterx-3*chasiswidth/4, boxcentery+3*chasisheight/4 + 3);
			wheel = m_world->CreateBody(&bdwheel); 
			wheel->CreateFixture(&fdwheel);
			
			bdwheel.position.Set(boxcenterx-3*chasiswidth/4, boxcentery-3*chasisheight/4 - 3);
			wheel = m_world->CreateBody(&bdwheel); 
			wheel->CreateFixture(&fdwheel);
			
			shape.SetAsBox(4.0f, 2);
			fdwheel.shape = &shape;
			bdwheel.position.Set(boxcenterx + chasiswidth/4 + 5, chasisheight+3 + boxcentery);
			wheel = m_world->CreateBody(&bdwheel); 
			wheel->CreateFixture(&fdwheel);
	...
	* */
	...
	{
		b2PolygonShape shape;
		shape.SetAsBox(0.1f,chasisheight - 3.5);
		b2FixtureDef fdbox;
		fdbox.shape = &shape;
	...
	* */
	...
	/*! <ul>
	* <li> Object Name: exhaust pipe</li>
	* <li> Object Shape: b2PolygonShape </li>
	* <li> Fixture : <ol>
	* 				<li> filter.maskBits=0x1000</li>
	* 				<li> filter.categoryBits=0x1000</li>
	...
	* <li> type : staticBody/li>
	* <li> Fuel particles are moved from piston chamber and are destroyed in this when the valve touches the sensor </li>
	* </ul>
	* */
	b2Body* exhaustpipe;
	{
		...
		//~ exhaustpipe->SetTransform(b2Vec2(boxcenterx+25,boxcentery+16), -45*DEGTORAD);
		//~ shape.SetAsBox(0.1,9.0,b2Vec2(3,-2),0);
		//~ exhaustpipe->CreateFixture(&fd);
		//~ 
		//~ exhaustpipe->SetTransform(b2Vec2(boxcenterx+16,boxcentery+16), -45*DEGTORAD);
		
		b2PolygonShape shape;
		shape.SetAsBox(0.1,9);
		b2FixtureDef fd;
		fd.shape=&shape;
		b2BodyDef bd;
		...
		exhaustpipe=m_world->CreateBody(&bd);
		exhaustpipe->CreateFixture(&fd);
		exhaustpipe->SetTransform(b2Vec2(boxcenterx+16.2,boxcentery+14), -40*DEGTORAD);
		
		b2Body* exhaustpipe2;
		shape.SetAsBox(0.1,8.4);
		fd.shape=&shape;
		exhaustpipe2=m_world->CreateBody(&bd);
		exhaustpipe2->CreateFixture(&fd);
		exhaustpipe2->SetTransform(b2Vec2(boxcenterx+18.8,boxcentery+13), -30*DEGTORAD);
	}
	//the exhaust
	{
		b2PolygonShape shape;
		shape.SetAsBox(2.0,0.2);
		b2FixtureDef fd;
		fd.shape=&shape;
		fd.isSensor=true;
		fd.filter.maskBits=0x8000;
		fd.filter.categoryBits=0x1000;
		b2BodyDef bd;
		bd.position.Set(boxcenterx+15,boxcentery+10);
	...
	* <li> type : staticBody/li>
	* <li> It is situated to the left of the circle and when the circle rotates it sense it.</li>
	* </ul>
	* */
	b2Body * rotSensor;
	{
	...
	* */
	...
	{
		b2PolygonShape shape;
		shape.SetAsBox(0.75,2.3,b2Vec2(0,0),70*DEGTORAD);
		b2FixtureDef fd;
		fd.shape=&shape;
		fd.filter.maskBits=0x8000;
		fd.filter.categoryBits=0x1000;
		b2BodyDef bd;
		bd.position.Set(boxcenterx+12.8,boxcentery+6.9);
	...
	* */
	...
	/*! \subsection subsec20 Stroke sensors
	 * \brief 
	 * */
	 
	/*! <ul>
	* <li> Object Name: Stroke_2 , Stroke_3 , Stroke_4 sensors</li>
	* <li> Object Shape: b2CircleShape circleShape </li>
	* <li> Fixture : <ol>
	* 				<li> stroke 2 filter.maskBits=0x0100</li>
	* 				<li> stroke 3 filter.maskBits=0x0100</li>
	* 				<li> stroke 4 filter.maskBits=0x0100</li>
	* 				<li> isSensor=true</li>
	* 				</ol></li>
	* <li> Position-stroke_2: boxcenterx+8.8f, 16.7+height_support</li>
	* <li> Position-stroke_3: boxcenterx+10.9f, 16.0f+height_support</li>
	* <li> Position-stroke_4: boxcenterx+11.3f, 18+height_support</li>
	* <li> type : staticBody</li>
	* <li> 
	* 
	* </li>
	...
	* */
	...
	/*! \subsection subsec21 Head Circle
...
/** These three circle composite object move the bars which in turn move the piston */
	...
	/*! \subsection subsec22 Small Head Circle (Left and Right)
	 * \brief 
	 * */
	 
	/*! <ul>
	* <li> Object Name: Small Head Circle(Left and Right)</li>
	* <li> Object Shape: b2CircleShape smallCircleShape </li>
	* <li> Fixture : <ol>
	* 				<li> radius = 0.8;</li>
	* 				<li> density = 5.0;</li>
	* 				<li> friction=10;</li>
	* 				<li> Category Bits(Left Circle) = 0x0100</li>
	* 				<li> Category Bits(Right Circle) = 0x0200</li>
	* 				</ol></li>
	* <li> Left Circle Position: boxcenterx+10.0, 18.75f + height_support</li>
	* <li> Right Circle Position: boxcenterx+10.0f, 18.7f + height_support</li>
	* <li> type : dynamicBody</li>
	* <li>This Circle causes the roataion of the composite joint and 
	* then also causes the movement of the piston in proper direction Left and right Pistons are moved using this.
	* This circle is attached to the head circle with a joint to rotate it. </li>
	* </ul>
	* */
	...
	/*! \subsection subsec23 Middle Trep
	 * \brief 
	 * */
	 
	/*! <ul>
	* <li> Object Name: Trep in the middle</li>
	* <li> Object Shape:  b2PolygonShape shapeTrep;</li>
	* <li> Fixture : <ol>
	* 				<li> VecType = b2Vec2 verticetp[3];</li>
	* 				<li>maskBits=0x8000;</li>
	* 				<li>categoryBits=0x1000;</li>
	* 				<li> density = 100.0f;</li>
	* 				</ol></li>
	* <li> Middle Trep Position : boxcenterx+9.05f, 12.0f + height_support</li>
	* <li> type : staticBody</li>
	* <li>static body between two valve to prevent them from 
	*  colliding and giving them proper direction </li>
	* </ul>
	* */
	 ...
	 * Hinge of the first left joint 
	 * that prevents it from randomly rotating be any angle*/
	...
	/*! \subsection subsec24 Left and Right Joints(First)
	 * \brief 
	 * */
	 
	/*! <ul>
	* <li> Object Name: First Joints</li>
	* <li> Object Shape:  b2PolygonShape shapelj;;</li>
	* <li> Fixture : <ol>
	* 				<li>categoryBits(First Left Joint)=0x1000;</li>
	* 				<li> density(First Left Joint) = 1.0f;</li>
	* 				<li>maskBits(First Left Joint)=0x0100;</li>
	* 				<li>restitution(First Left Joint) = 2.5;</li>
	* 
	* 				<li>categoryBits(First Right Joint)=0x2000;</li>
	* 				<li> density(First Right Joint) = 1.0f;</li>
	* 				<li>maskBits(First Right Joint)=0x0200;</li>
	* 				<li>restitution(First Right Joint) = 2.5;</li>
	* 				</ol></li>
	* <li> First Left Joint Position: boxcenterx+6.5f, 19.0f + height_support </li>
	* <li> First Left Joint Position: boxcenterx+13.5f, 19.0f + height_support </li>
	* <li> type : dynamicBody</li>
	* <li>These joints are the main thing that drives the motion of left and right valves respectively.The joint
	* is revoluted and the hinge prevents the rotation . Static bodies on the circle stops the motion and send the joints back 
	* to their original positions</li>
	* </ul>
	* */
	...
	/*! \subsection subsec25 Left and Right Joints(Second)
	 * \brief 
	 * */
	
	/*! <ul>
	* <li> Object Name: Second Joints</li>
	* <li> Object Shape:  b2PolygonShape shapelj2;;</li>
	* <li> Fixture : <ol>
	* 				<li> density(Second Left Joint) = 1.0f;</li>
	* 				<li>maskBits(Second Left Joint)=0x0100;</li>
	* 				<li> density(Second Right Joint) = 1.0f;</li>
	* 				<li>maskBits(Second Right Joint)=0x0200;</li>
	* 				</ol></li>
	* <li> Second Left Joint Position: boxcenterx+6.5f, 19.0f + height_support </li>
	* <li> Second Right Joint Position: boxcenterx+13.5f, 19.0f + height_support </li>
	* <li> type : dynamicBody</li>
	* <li>These joints are attached to the first joints and sets the valve in linear motion </li>
	* </ul>
	* */
	...
	/*! \subsection subsec26 Rest Body(Left and Right)
	 * \brief 
	 * */
	 
	/*! <ul>
	* <li> Object Name: Rest Circles</li>
	* <li> Object Shape:  b2BodyDef smallCircleRight;</li>
	* <li> Fixture : <ol>
	* 				<li> density(Third Left Joint) = 1.0f; </li>
	* 				<li> radius = 0.5; </li>
	* 				<li> categoryBits=0x0200; </li>
	* 				</ol></li>
	* <li> Left Rest Circle Position: boxcenterx+6.5f, 16.5f + height_support </li>
	* <li> Right Rest Circle Position: boxcenterx+13.5f, 16.5f + height_support </li>
	* <li> type : staticBody</li>
	* <li>Rest circles are used to constraint the motion of joints, so that the valve does not move too much. </li>
	* </ul>
	* */
	...
	/*! \subsection subsec27 Left and Right Joints(Third)
	 * \brief 
	 * */
	 
	/*! <ul>
	* <li> Object Name: Third Joints</li>
	* <li> Object Shape:  b2PolygonShape shapelj3;</li>
	* <li> Fixture : <ol>
	* 				<li> density(Third Left Joint) = 1.0f;</li>
	* 				<li>maskBits(Third Left Joint)=0x0100;</li>
	* 				<li>restitution(Third Left Joint) = 2.5;</li>
	* 				<li> density(Third Right Joint) = 1.0f;</li>
	* 				<li>maskBits(Third Right Joint)=0x0200;</li>
	* 				<li>restitution(Third Right Joint) = 2.5;</li>
	* 				</ol></li>
	* <li> Third Left Joint: boxcenterx+5.5f, 17.5f + height_support </li>
	* <li> Third Right Joint: boxcenterx+14.5f, 17.5f + height_support </li>
	* <li> type : dynamicBody</li>
	* <li> </li>
	* </ul>
	* */
	...
	/*! \subsection subsec28 Rod at Top(Third)
	 * \brief 
	 * */
	 
	/*! <ul>
	* <li> Object Name: Rod at top of valve</li>
	* <li> Object Shape:  b2PolygonShape shaperod;</li>
	* <li> Fixture : <ol>
	* 				<li> density = 1.0f;</li>
	* 				<li>maskBits=0x0100;</li>
	* 				</ol></li>
	* <li> Rod at Top Position: boxcenterx+5.00f, 15.5f + height_support </li>
	* <li> type : dynamicBody</li>
	* <li>The rod is located at the top of the valve. A Hinge is fixed to the rod so that it moves perpendicular to one direction
	* Get a prismatic joint at that place</li>
	* </ul>
	* */
	...
	/*! \subsection subsec29 Fuel Valve (Incoming and Outgoing)
	 * \brief 
	 * */
	 
	/*! <ul>
	* <li> Object Name: Fuel Valves</li>
	* <li> Object Shape:  b2PolygonShape shapePoly;</li>
	* <li> Fixture : <ol>
	* 				<li> maskBits = 0x8000; </li>
	* 				<li> categoryBits=0x1000; </li>
	* 				<li> density = 0.5f; </li>
	...
	* <li>  </li>
	* <li> type : dynamicBody</li>
	* <li> Fuel Valve(incoming) Position : boxcenterx+5.8f, 18.1f+height_support/2 </li>
	* <li> Fuel Valve(outgoing) Position : boxcenterx+14.2f, 17.1f+height_support/2 </li>
	* <li>The valve for the incoming and outgoing of fuel.Joint between the valve and the rod left side, this 
	* is a fixed joint that forbid revolution . similarly on the right-side.</li>
	* </ul>
	* */

...
int main(int argc, char** argv)
{
  ...
  test = entry->create_fcn();
	//---------------------------------------------------------------------------------
	//~ b2World* n_world=test->get_world();
   //~ const b2Profile& p =n_world->GetProfile();
  //~ float32 time_step = settings.hz > 0.0f ? 1.0f / settings.hz : float32(0.0f);
//~ 
  //~ b2Timer timer;
  //~ float step=0, collide=0, solveVelocity=0,solvePosition=0;
  //~ for(int j=0;j<iter;j++){
   	//~ n_world->Step(time_step, settings.velocity_iterations, settings.position_iterations);
   	//~ step+=p.step;
   	//~ collide+=p.collide;
  ...
  //~ }
	...
	//~ std::cout<<"Average time per step is "<<step/iter<<" ms"<<std::endl;
	//~ std::cout<<"Average time for collisions is "<<collide/iter<<" ms"<<std::endl;
	//~ std::cout<<"Average time for velocity updates is "<<solveVelocity/iter<<" ms"<<std::endl;
	//~ std::cout<<"Average time for position updates is "<<solvePosition/iter<<" ms"<<std::endl;
	//~ std::cout<<"Total loop time "<<timer.GetMilliseconds()<<" ms"<<std::endl;
